...
public interface OffGroupService {
	// 그룹 등록
	void registOffGroup(OffGroup offGroup);

	// 그룹 수정
	void updateOffGroup(OffGroup offGroup);

	// 그룹 삭제
	void deleteOffGroup(int group_id);

	// 그룹 셀렉트
	List<OffGroup> selectOffGroupList();
	...
	//유저 아이디로 참가한 그룹 가져오기
	List<OffGroup> getMyGroup(String user_id);
	// 그룹에 참가버튼을 누른 사람이 몇명인지 반환하는 메서드
	int countJoin(OffGroup offGroup);
}
