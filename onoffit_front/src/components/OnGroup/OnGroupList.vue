...
<script>
import OnGroupListRow from "./Include/OnGroupListRow.vue";
import { mapState } from "vuex";
export default {
  computed: {
    ...mapState(["onGroupList"]),
    totalPages() {
      return Math.ceil(this.onGroupList.length / this.itemsPerPage); // 전체 페이지 수 계산
    },
    paginatedList() {
      const startIndex = (this.currentPage - 1) * this.itemsPerPage; // 시작 인덱스 계산
      const endIndex = startIndex + this.itemsPerPage; // 끝 인덱스 계산
      return this.onGroupList.slice(startIndex, endIndex); // 현재 페이지에 해당하는 목록 반환
    },
  },
  data() {
    return {
  ...
  mounted() {
    this.$store.dispatch("getOnGroup");
  },
  components: {
    OnGroupListRow,
  },
